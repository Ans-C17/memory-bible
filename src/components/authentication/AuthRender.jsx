...
function AuthRender({ user, setUser }) {
  ...
  if (user) {
    return (
      <div className="h-64 w-72 absolute right-10 border-2 border-cyan-300 rounded-2xl p-6 bg-gray-800 flex flex-col">
        <HeyUser user={user} />
      </div>
    );
  } else if (!showSignUp) {
    ...
    return (
      <>
        <div className="h-72 w-72 absolute right-10 border-2 border-cyan-300 rounded-t-2xl p-6 bg-gray-800 flex flex-col">
          <h2 className="text-2xl text-emerald-200 font-anek text-center mb-6">
            LOGIN
          </h2>
          <Login setUser={setUser} />
        </div>
        <div className="w-72 h-16 absolute space-x-2 top-82 right-10 flex items-center justify-center border-2 border-cyan-300 rounded-b-2xl bg-gray-800 font-anek text-lg">
